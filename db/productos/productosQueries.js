...
const crearProductoQuery = async (producto) => {
    const { codigo, nombre, descripcion, precio_costo, precio_unitario, stock_actual, stock_minimo, unidad_de_medida} = producto;
    return new Promise((resolve, reject) => {
        const sql = 'INSERT INTO productos (nombre, codigo, precio, disponible) VALUES (?, ?, ?, ?)';
        config.query(sql, [codigo, nombre, descripcion, precio_costo, precio_unitario, stock_actual, stock_minimo, unidad_de_medida], (err, resultado) => {
            if (err) {
                reject(err);
            } else {
                resolve(resultado);
